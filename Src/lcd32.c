...
extern const unsigned char asc2_3216[1][32];
extern const unsigned char icon_demo[63720];
extern const unsigned char BIG_ASCII[195][16];

extern const unsigned int image_data_rgb[24000];
...
void lcd32_draw_line(u16 x1, u16 y1, u16 x2, u16 y2) {
	...
	uCol = y1;
	if (delta_x > 0)
		incx = 1; // establishes on foot the direction
	else if (delta_x == 0)
	...
	if (delta_x > delta_y)
		distance = delta_x; // selects the basic increase coordinate axis
	else
		distance = delta_y;
...
#define MAX_CHAR28_POSY 304 
	...
	} else		 //superimposition way
	{
		for (pos = 0; pos < size; pos++) {
			...
			for (t = 0; t < size / 2; t++) {
				if (temp & 0x01)
					lcd32_draw_point(x + t, y + pos); // draws a spot
				temp >>= 1;
			}
		}
	}
